...
function sha1(file, callback) {
    var reader = new FileReader, _rush = new Rusha;
    reader.onprogress = function (evt) {
        var $scope = angular.element(document.body).scope();
        $scope.library.progress = Math.round(evt.loaded / evt.total * 10)
    }, reader.onload = function () {
        var chunk = new Uint8Array(reader.result, 0, file.size), uuid = _rush.digestFromBuffer(chunk);
        callback(uuid, file)
    }, reader.readAsArrayBuffer(file)
}
function openFiles($scope, files) {
    function loopFiles(files) {
        "epub" == files[i].name.split(".").pop().toLowerCase() && ($scope.library.loading = !0, sha1(files[i], function (uuid, file) {
            filer.cd("/", function () {
                filer.write("/readiator/epub/" + uuid + ".epub", {
                    data: file,
                    type: file.type
                }, function (fileEntry, fileWriter) {
                }, onError), filer.mkdir("/readiator/books/" + uuid + ".epub/", !1, function (dirEntry) {
                    $scope.library.progress = 0;
                    var zipFs = new zip.fs.FS;
                    zipFs.importBlob(file, function () {
                        zipFs.root.getFileEntry(dirEntry, function () {
                            $scope.library.loading = !1, scanBooks($scope), i++, i < files.length && loopFiles(files)
                        }, function (p, max) {
                            $scope.library.progress = Math.round(p / max * 100), $scope.$apply()
                        }, onError)
                    }, function () {
                        filer.rm(dirEntry.fullPath, function () {
                        }, onError)
                    })
                }, onError)
            }, onError)
    ...
    if (console.log(files), files.length > 0) {
        var i = 0;
        loopFiles(files)
    }
}
function scanBooks($scope) {
    $scope.library.books = [], filer.cd("/", function (entries) {
        filer.ls("/readiator/books/", function (entries) {
            entries.forEach(function (entry) {
                if (entry.isDirectory) {
                    var url = "filesystem:" + location.protocol + "//" + location.host + "/persistent" + entry.fullPath + "/", book = ePub(url, {restore: !1});
                    book.on("book:ready", function () {
                        ...
                        }), $scope.$apply(), componentHandler.upgradeDom("MaterialMenu", "mdl-menu")
                    })
                }
            }), $scope.$apply()
        }, onError)
...
function googleAnalytics() {
    ...
    tracker.sendAppView("index"), tracker.sendEvent("Browsing", "Browsed the library")
}
!function () {
    "use strict";
    function e(e, t) {
    ...
    function qFactory(nextTick, exceptionHandler) {
        ...
        var $qMinErr = minErr("$q", TypeError), defer = function () {
            var d = new Deferred;
            return d.resolve = simpleBind(d, d.resolve), d.reject = simpleBind(d, d.reject), d.notify = simpleBind(d, d.notify),
                d
        };
    ...
    forEach({
        ...
        removeData: jqLiteRemoveData, on: function (element, type, fn, unsupported) {
...
var self = this;
if (self.URL = self.URL || self.webkitURL, self.requestFileSystem = self.requestFileSystem || self.webkitRequestFileSystem, self.resolveLocalFileSystemURL = self.resolveLocalFileSystemURL || self.webkitResolveLocalFileSystemURL, navigator.temporaryStorage = navigator.temporaryStorage || navigator.webkitTemporaryStorage, navigator.persistentStorage = navigator.persistentStorage || navigator.webkitPersistentStorage, self.BlobBuilder = self.BlobBuilder || self.MozBlobBuilder || self.WebKitBlobBuilder, void 0 === self.FileError) {
...
var Util = {
    toArray: function (list) {
        ...
        var isBinary = void 0 != opt_isBinary ? opt_isBinary : !0;
        return isBinary ? "data:" + contentType + ";base64," + self.btoa(str) : "data:" + contentType + "," + str
    }, strToObjectURL: function (binStr, opt_contentType) {
        for (var ui8a = new Uint8Array(binStr.length), i = 0; i < ui8a.length; ++i)ui8a[i] = binStr.charCodeAt(i);
        var blob = new Blob([ui8a], opt_contentType ? {type: opt_contentType} : {});
        ...
        if (-1 == dataURL.indexOf(BASE64_MARKER)) {
            var parts = dataURL.split(","), contentType = parts[0].split(":")[1], raw = parts[1];
            return new Blob([raw], {type: contentType})
        }
        for (var parts = dataURL.split(BASE64_MARKER), contentType = parts[0].split(":")[1], raw = window.atob(parts[1]), rawLength = raw.length, uInt8Array = new Uint8Array(rawLength), i = 0; rawLength > i; ++i)uInt8Array[i] = raw.charCodeAt(i);
        return new Blob([uInt8Array], {type: contentType})
    }, arrayBufferToBlob: function (buffer, opt_contentType) {
        var uInt8Array = new Uint8Array(buffer);
        return new Blob([uInt8Array], opt_contentType ? {type: opt_contentType} : {})
        ...
        for (var i = bytes.length, bstr = new Array(i); i--;)bstr[i] = String.fromCharCode(bytes[i]);
        return bstr.join("")
    }, getFileExtension: function (filename) {
        var idx = filename.lastIndexOf(".");
        return -1 != idx ? filename.substring(idx) : ""
    }
}, MyFileError = function (obj) {
    this.prototype = FileError.prototype, this.code = obj.code, this.name = obj.name
};
FileError.BROWSER_NOT_SUPPORTED = 1e3, FileError.prototype.__defineGetter__("name", function () {
    for (var key, keys = Object.keys(FileError), i = 0; key = keys[i]; ++i)if (FileError[key] == this.code)return key;
    return "Unknown Error"
});
var Filer = new function () {
    function Filer(fs) {
        fs_ = fs || null, fs_ && (cwd_ = fs_.root, isOpen_ = !0)
    }

    var FS_INIT_ERROR_MSG = "Filesystem has not been initialized.", NOT_IMPLEMENTED_MSG = "Not implemented.", NOT_A_DIRECTORY = "Path was not a directory.", INCORRECT_ARGS = "These method arguments are not supported.", FS_URL_SCHEME = "filesystem:", DEFAULT_FS_SIZE = 1048576, fs_ = null, cwd_ = null, isOpen_ = !1, isFsURL_ = function (path) {
        return 0 == path.indexOf(FS_URL_SCHEME)
    }, pathToFsURL_ = function (path) {
        return isFsURL_(path) || (path = "/" == path[0] ? fs_.root.toURL() + path.substring(1) : 0 == path.indexOf("./") || 0 == path.indexOf("../") ? "../" == path && cwd_ != fs_.root ? cwd_.toURL() + "/" + path : cwd_.toURL() + path : cwd_.toURL() + "/" + path), path
    }, getEntry_ = function (callback, var_args) {
        var srcStr = arguments[1], destStr = arguments[2], onError = function (e) {
            throw e.code == FileError.NOT_FOUND_ERR ? destStr ? new Error('"' + srcStr + '" or "' + destStr + '" does not exist.') : new Error('"' + srcStr + '" does not exist.') : new Error("Problem getting Entry for one or more paths.")
        }, src = pathToFsURL_(srcStr);
    ...
    }, copyOrMove_ = function (src, dest, opt_newName, opt_successCallback, opt_errorHandler, opt_deleteOrig) {
        if (!fs_)throw new Error(FS_INIT_ERROR_MSG);
        if (typeof src != typeof dest)throw new Error(INCORRECT_ARGS);
        var newName = opt_newName || null, deleteOrig = void 0 != opt_deleteOrig ? opt_deleteOrig : !1;
        (src.isFile || dest.isDirectory) && dest.isDirectory ? deleteOrig ? src.moveTo(dest, newName, opt_successCallback, opt_errorHandler) : src.copyTo(dest, newName, opt_successCallback, opt_errorHandler) : getEntry_(function (srcEntry, destDir) {
            if (!destDir.isDirectory) {
                var e = new Error('Oops! "' + destDir.name + " is not a directory!");
                if (!opt_errorHandler)throw e;
                return void opt_errorHandler(e)
            }
            deleteOrig ? srcEntry.moveTo(destDir, newName, opt_successCallback, opt_errorHandler) : srcEntry.copyTo(destDir, newName, opt_successCallback, opt_errorHandler)
        }, src, dest)
    };
    return Filer.DEFAULT_FS_SIZE = DEFAULT_FS_SIZE, Filer.version = "0.4.3", Filer.prototype = {
        get fs() {
            return fs_
        }, get isOpen() {
            return isOpen_
        ...
        var initObj = opt_initObj ? opt_initObj : {}, size = initObj.size || DEFAULT_FS_SIZE;
        this.type = self.TEMPORARY, "persistent" in initObj && initObj.persistent && (this.type = self.PERSISTENT);
        var init = function (fs) {
            this.size = size, fs_ = fs, cwd_ = fs_.root, isOpen_ = !0, opt_successCallback && opt_successCallback(fs)
        };
        this.type == self.PERSISTENT && navigator.persistentStorage ? navigator.persistentStorage.requestQuota(size, function (grantedBytes) {
            self.requestFileSystem(this.type, grantedBytes, init.bind(this), opt_errorHandler)
        }.bind(this), opt_errorHandler) : self.requestFileSystem(this.type, size, init.bind(this), opt_errorHandler)
    }, Filer.prototype.ls = function (dirEntryOrPath, successCallback, opt_errorHandler) {
        if (!fs_)throw new Error(FS_INIT_ERROR_MSG);
        var callback = function (dirEntry) {
            cwd_ = dirEntry;
            var entries_ = [], reader = cwd_.createReader(), readEntries = function () {
                reader.readEntries(function (results) {
                    results.length ? (entries_ = entries_.concat(Util.toArray(results)), readEntries()) : (entries_.sort(function (a, b) {
        ...
        dirEntryOrPath.isDirectory ? callback(dirEntryOrPath) : isFsURL_(dirEntryOrPath) ? getEntry_(callback, pathToFsURL_(dirEntryOrPath)) : cwd_.getDirectory(dirEntryOrPath, {}, callback, opt_errorHandler)
    }, Filer.prototype.mkdir = function (path, opt_exclusive, opt_successCallback, opt_errorHandler) {
        if (!fs_)throw new Error(FS_INIT_ERROR_MSG);
        var exclusive = null != opt_exclusive ? opt_exclusive : !1, folderParts = path.split("/"), createDir = function (rootDir, folders) {
            "." != folders[0] && "" != folders[0] || (folders = folders.slice(1)), rootDir.getDirectory(folders[0], {
                create: !0,
                exclusive: exclusive
            }, function (dirEntry) {
                if (dirEntry.isDirectory)folders.length && 1 != folderParts.length ? createDir(dirEntry, folders.slice(1)) : opt_successCallback && opt_successCallback(dirEntry); else {
                ...
                if (e.code == FileError.INVALID_MODIFICATION_ERR) {
                    if (e.message = "'" + path + "' already exists", !opt_errorHandler)throw e;
                    opt_errorHandler(e)
                }
            })
        };
        ...
        if (!fs_)throw new Error(FS_INIT_ERROR_MSG);
        entryOrPath.isFile ? entryOrPath.file(successCallback, opt_errorHandler) : getEntry_(function (fileEntry) {
        ...
        if (!fs_)throw new Error(FS_INIT_ERROR_MSG);
        var exclusive = null != opt_exclusive ? opt_exclusive : !0;
        cwd_.getFile(path, {create: !0, exclusive: exclusive}, successCallback, function (e) {
            if (e.code == FileError.INVALID_MODIFICATION_ERR && (e.message = "'" + path + "' already exists"), !opt_errorHandler)throw e;
            opt_errorHandler(e)
        })
    }, Filer.prototype.mv = function (src, dest, opt_newName, opt_successCallback, opt_errorHandler) {
        copyOrMove_.bind(this, src, dest, opt_newName, opt_successCallback, opt_errorHandler, !0)()
    }, Filer.prototype.rm = function (entryOrPath, successCallback, opt_errorHandler) {
        if (!fs_)throw new Error(FS_INIT_ERROR_MSG);
        var removeIt = function (entry) {
            entry.isFile ? entry.remove(successCallback, opt_errorHandler) : entry.isDirectory && entry.removeRecursively(successCallback, opt_errorHandler)
        };
        entryOrPath.isFile || entryOrPath.isDirectory ? removeIt(entryOrPath) : getEntry_(removeIt, entryOrPath)
    }, Filer.prototype.cd = function (dirEntryOrPath, opt_successCallback, opt_errorHandler) {
        if (!fs_)throw new Error(FS_INIT_ERROR_MSG);
        if (dirEntryOrPath.isDirectory)cwd_ = dirEntryOrPath, opt_successCallback && opt_successCallback(cwd_); else {
            var dirEntryOrPath = pathToFsURL_(dirEntryOrPath);
            getEntry_(function (dirEntry) {
                if (dirEntry.isDirectory)cwd_ = dirEntry, opt_successCallback && opt_successCallback(cwd_); else {
        ...
        var writeFile_ = function (fileEntry) {
            fileEntry.createWriter(function (fileWriter) {
                if (fileWriter.onerror = opt_errorHandler, dataObj.append)fileWriter.onwriteend = function (e) {
                    opt_successCallback && opt_successCallback(fileEntry, this)
                }, fileWriter.seek(fileWriter.length); else {
                    var truncated = !1;
                    fileWriter.onwriteend = function (e) {
                        return truncated ? void(opt_successCallback && opt_successCallback(fileEntry, this)) : (truncated = !0, void this.truncate(this.position))
                    }
                }
                dataObj.data.__proto__ == ArrayBuffer.prototype && (dataObj.data = new Uint8Array(dataObj.data));
                var blob = new Blob([dataObj.data], dataObj.type ? {type: dataObj.type} : {});
                fileWriter.write(blob)
            }, opt_errorHandler)
        };
        entryOrPath.isFile ? writeFile_(entryOrPath) : isFsURL_(entryOrPath) ? getEntry_(writeFile_, entryOrPath) : cwd_.getFile(entryOrPath, {
            create: !0,
            exclusive: !1
        ...
        self.TEMPORARY == this.type ? navigator.temporaryStorage.queryUsageAndQuota(queryCallback, opt_errorHandler) : self.PERSISTENT == this.type && navigator.persistentStorage.queryUsageAndQuota(queryCallback, opt_errorHandler)
    }, Filer
};
!function (obj) {
    "use strict";
    ...
    function blobSlice(blob, index, length) {
        if (0 > index || 0 > length || index + length > blob.size)throw new RangeError("offset:" + index + ", length:" + length + ", size:" + blob.size);
    ...
    function Data64URIReader(dataURI) {
        ...
        function readUint8Array(index, length, callback) {
            ...
            for (i = delta; delta + length > i; i++)data.array[i - delta] = bytes.charCodeAt(i);
            callback(data.array)
        }
    ...
    function TextWriter(encoding) {
        function init(callback) {
            blob = new Blob([], {type: TEXT_PLAIN}), callback()
        }

        function writeUint8Array(array, callback) {
            blob = new Blob([blob, appendABViewSupported ? array : array.buffer], {type: TEXT_PLAIN}), callback()
        }
    ...
    function BlobWriter(contentType) {
        function init(callback) {
            blob = new Blob([], {type: contentType}), callback()
        }

        function writeUint8Array(array, callback) {
            blob = new Blob([blob, appendABViewSupported ? array : array.buffer], {type: contentType}), callback()
        }
    ...
    function launchWorkerProcess(worker, initialMessage, reader, writer, offset, size, onprogress, onend, onreaderror, onwriteerror) {
        ...
        function step() {
            index = chunkIndex * CHUNK_SIZE, size >= index ? reader.readUint8Array(offset + index, Math.min(CHUNK_SIZE, size - index), function (array) {
                onprogress && onprogress(index, size);
                var msg = 0 === index ? initialMessage : {sn: sn};
                ...
                chunkIndex++
            }, onreaderror) : worker.postMessage({sn: sn, type: "flush"})
        }
    ...
    function launchProcess(process, reader, writer, offset, size, crcType, onprogress, onend, onreaderror, onwriteerror) {
        function step() {
            var outputData;
            if (index = chunkIndex * CHUNK_SIZE, size > index)reader.readUint8Array(offset + index, Math.min(CHUNK_SIZE, size - index), function (inputData) {
                var outputData;
                try {
    ...
    function inflate(worker, sn, reader, writer, offset, size, computeCrc32, onend, onprogress, onreaderror, onwriteerror) {
        ...
        if (obj.zip.useWebWorkers) {
            var initialMessage = {sn: sn, codecClass: "Inflater", crcType: crcType};
    ...
    function deflate(worker, sn, reader, writer, level, onend, onprogress, onreaderror, onwriteerror) {
        var crcType = "input";
        if (obj.zip.useWebWorkers) {
            var initialMessage = {sn: sn, options: {level: level}, codecClass: "Deflater", crcType: crcType};
    ...
    function copy(worker, sn, reader, writer, offset, size, computeCrc32, onend, onprogress, onreaderror, onwriteerror) {
        ...
        if (obj.zip.useWebWorkers && computeCrc32) {
            var initialMessage = {sn: sn, codecClass: "NOOP", crcType: crcType};
            launchWorkerProcess(worker, initialMessage, reader, writer, offset, size, onprogress, onend, onreaderror, onwriteerror)
        } else launchProcess(new NOOP, reader, writer, offset, size, crcType, onprogress, onend, onreaderror, onwriteerror)
    }

    function decodeASCII(str) {
        var i, charCode, out = "", extendedASCII = ["Ç", "ü", "é", "â", "ä", "à", "å", "ç", "ê", "ë", "è", "ï", "î", "ì", "Ä", "Å", "É", "æ", "Æ", "ô", "ö", "ò", "û", "ù", "ÿ", "Ö", "Ü", "ø", "£", "Ø", "×", "ƒ", "á", "í", "ó", "ú", "ñ", "Ñ", "ª", "º", "¿", "®", "¬", "½", "¼", "¡", "«", "»", "_", "_", "_", "¦", "¦", "Á", "Â", "À", "©", "¦", "¦", "+", "+", "¢", "¥", "+", "+", "-", "-", "+", "-", "+", "ã", "Ã", "+", "+", "-", "-", "¦", "-", "+", "¤", "ð", "Ð", "Ê", "Ë", "È", "i", "Í", "Î", "Ï", "+", "+", "_", "_", "¦", "Ì", "_", "Ó", "ß", "Ô", "Ò", "õ", "Õ", "µ", "þ", "Þ", "Ú", "Û", "Ù", "ý", "Ý", "¯", "´", "­", "±", "_", "¾", "¶", "§", "÷", "¸", "°", "¨", "·", "¹", "³", "²", "_", " "];
    ...
    function createZipReader(reader, callback, onerror) {
        ...
        function seekEOCDR(eocdrCallback) {
            ...
            var ZIP_COMMENT_MAX = 65536, EOCDR_MAX = EOCDR_MIN + ZIP_COMMENT_MAX;
        ...
        var zipReader = {
            getEntries: function (callback) {
                ...
                seekEOCDR(function (dataView) {
                    ...
                    return datalength = dataView.getUint32(16, !0), fileslength = dataView.getUint16(8, !0), 0 > datalength || datalength >= reader.size ? void onerror(ERR_BAD_FORMAT) : void reader.readUint8Array(datalength, reader.size - datalength, function (bytes) {
                        var i, entry, filename, comment, index = 0, entries = [], data = getDataHelper(bytes.length, bytes);
                        for (i = 0; fileslength > i; i++) {
    ...
    Crc32.prototype.append = function (data) {
        for (var crc = 0 | this.crc, table = this.table, offset = 0, len = 0 | data.length; len > offset; offset++)crc = crc >>> 8 ^ table[255 & (crc ^ data[offset])];
        this.crc = crc
        ...
        var i, j, t, table = [];
        for (i = 0; 256 > i; i++) {
            for (t = i, j = 0; 8 > j; j++)1 & t ? t = t >>> 1 ^ 3988292384 : t >>>= 1;
            table[i] = t
        }
        return table
    ...
    }, TextReader.prototype = new Reader, TextReader.prototype.constructor = TextReader, Data64URIReader.prototype = new Reader, Data64URIReader.prototype.constructor = Data64URIReader, BlobReader.prototype = new Reader, BlobReader.prototype.constructor = BlobReader, Writer.prototype.getData = function (callback) {
        callback(this.data)
    ...
    var DEFAULT_WORKER_SCRIPTS = {deflater: ["z-worker.js", "deflate.js"], inflater: ["z-worker.js", "inflate.js"]};
    obj.zip = {
        Reader: Reader,
        Writer: Writer,
    ...
    function ZipBlobReader(entry) {
        function init(callback) {
            that.size = entry.uncompressedSize, callback()
        }

        function getData(callback) {
            that.data ? callback() : entry.getData(new BlobWriter, function (data) {
                that.data = data, blobReader = new BlobReader(data), callback()
    ...
    function getTotalSize(entry) {
        function process(entry) {
            size += entry.uncompressedSize || 0, entry.children.forEach(process)
        }

        var size = 0;
        return process(entry), size
    }

    function initReaders(entry, onend, onerror) {
        function next() {
            index++, index < entry.children.length ? process(entry.children[index]) : onend()
        }

        function process(child) {
            child.directory ? initReaders(child, next, onerror) : (child.reader = new child.Reader(child.data, onerror), child.reader.init(function () {
                child.uncompressedSize = child.reader.size, next()
            }))
        }

        var index = 0;
        entry.children.length ? process(entry.children[index]) : onend()
    }

    function detach(entry) {
        var children = entry.parent.children;
        children.forEach(function (child, index) {
            child.id == entry.id && children.splice(index, 1)
        })
    }
    ...
    function exportZip(zipWriter, entry, onend, onprogress, totalSize) {
        function process(zipWriter, entry, onend, onprogress, totalSize) {
            function exportChild() {
                ...
                child ? zipWriter.add(child.getFullname(), child.reader, function () {
                    currentIndex += child.uncompressedSize || 0, process(zipWriter, child, function () {
                        childIndex++, exportChild()
                    }, onprogress, totalSize)
                }, function (index) {
                    onprogress && onprogress(currentIndex + index, totalSize)
                }, {directory: child.directory, version: child.zipVersion}) : onend()
            }
    ...
    function addFileEntry(zipEntry, fileEntry, onend, onerror) {
        function getChildren(fileEntry, callback) {
            fileEntry.isDirectory && fileEntry.createReader().readEntries(callback), fileEntry.isFile && callback([])
        }
                ...
                function addChild(child) {
                    function nextChild(childFileEntry) {
                        process(childFileEntry, child, function () {
                            childIndex++, processChild()
                        })
                    }

                    child.isDirectory && nextChild(zipEntry.addDirectory(child.name)), child.isFile && child.file(function (file) {
        ...
        fileEntry.isDirectory ? process(zipEntry, fileEntry, onend) : fileEntry.file(function (file) {
            zipEntry.addBlob(fileEntry.name, file), onend()
        }, onerror)
    }
    ...
    function getFileEntry(fileEntry, entry, onend, onprogress, onerror, totalSize, checkCrc32) {
        function process(fileEntry, entry, onend, onprogress, onerror, totalSize) {
            function addChild(child) {
                function nextChild(childFileEntry) {
                    currentIndex += child.uncompressedSize || 0, process(childFileEntry, child, function () {
                        childIndex++, processChild()
                    }, onprogress, onerror, totalSize)
                }

                child.directory ? fileEntry.getDirectory(child.name, {create: !0}, nextChild, onerror) : fileEntry.getFile(child.name, {create: !0}, function (file) {
        ...
        var currentIndex = 0;
        entry.directory ? process(fileEntry, entry, onend, onprogress, onerror, totalSize) : entry.getData(new zip.FileWriter(fileEntry, zip.getMimeType(entry.name)), onend, onprogress, checkCrc32)
    }

    function resetFS(fs) {
        fs.entries = [], fs.root = new ZipDirectoryEntry(fs)
    }

    function bufferedCopy(reader, writer, onend, onprogress, onerror) {
        function stepCopy() {
            var index = chunkIndex * CHUNK_SIZE;
            onprogress && onprogress(index, reader.size), index < reader.size ? reader.readUint8Array(index, Math.min(CHUNK_SIZE, reader.size - index), function (array) {
                writer.writeUint8Array(new Uint8Array(array), function () {
                    chunkIndex++, stepCopy()
                })
            }, onerror) : writer.getData(onend)
        }
    ...
    function ZipFileEntry(fs, name, params, parent) {
        var that = this;
        ZipEntry.prototype.init.call(that, fs, name, params, parent), that.Reader = params.Reader, that.Writer = params.Writer, that.data = params.data, params.getData && (that.getData = params.getData)
    }

    function ZipDirectoryEntry(fs, name, params, parent) {
        var that = this;
        ZipEntry.prototype.init.call(that, fs, name, params, parent), that.directory = !0
    }
    ...
    var CHUNK_SIZE = 524288, TextWriter = zip.TextWriter, BlobWriter = zip.BlobWriter, Data64URIWriter = zip.Data64URIWriter, Reader = zip.Reader, TextReader = zip.TextReader, BlobReader = zip.BlobReader, Data64URIReader = zip.Data64URIReader, createReader = zip.createReader, createWriter = zip.createWriter;
    ZipBlobReader.prototype = new Reader, ZipBlobReader.prototype.constructor = ZipBlobReader, ZipBlobReader.prototype.checkCrc32 = !1, ZipEntry.prototype = {
        init: function (fs, name, params, parent) {
            var that = this;
            if (fs.root && parent && parent.getChildByName(name))throw"Entry filename already exists.";
            params || (params = {}), that.fs = fs, that.name = name, that.id = fs.entries.length, that.parent = parent, that.children = [], that.zipVersion = params.zipVersion || 20, that.uncompressedSize = 0, fs.entries.push(that), parent && that.parent.children.push(that)
            ...
            if (target.isDescendantOf(that))throw"Entry is a ancestor of target entry.";
            if (that != target) {
                if (target.getChildByName(that.name))throw"Entry filename already exists.";
                detach(that), that.parent = target, target.children.push(that)
            }
        }, getFullname: function () {
            for (var that = this, fullname = that.name, entry = that.parent; entry;)fullname = (entry.name ? entry.name + "/" : "") + fullname, entry = entry.parent;
            return fullname
        }, isDescendantOf: function (ancestor) {
            for (var entry = this.parent; entry && entry.id != ancestor.id;)entry = entry.parent;
            return !!entry
        }
    }, ZipEntry.prototype.constructor = ZipEntry;
    var ZipFileEntryProto;
    ZipFileEntry.prototype = ZipFileEntryProto = new ZipEntry, ZipFileEntryProto.constructor = ZipFileEntry, ZipFileEntryProto.getData = function (writer, onend, onprogress, onerror) {
        var that = this;
        !writer || writer.constructor == that.Writer && that.data ? onend(that.data) : (that.reader || (that.reader = new that.Reader(that.data, onerror)), that.reader.init(function () {
    ...
    ZipDirectoryEntry.prototype = ZipDirectoryEntryProto = new ZipEntry, ZipDirectoryEntryProto.constructor = ZipDirectoryEntry, ZipDirectoryEntryProto.addDirectory = function (name) {
        return addChild(this, name, null, !0)
    }, ZipDirectoryEntryProto.addText = function (name, text) {
        return addChild(this, name, {data: text, Reader: TextReader, Writer: TextWriter})
    }, ZipDirectoryEntryProto.addBlob = function (name, blob) {
        return addChild(this, name, {data: blob, Reader: BlobReader, Writer: BlobWriter})
    }, ZipDirectoryEntryProto.addData64URI = function (name, dataURI) {
        return addChild(this, name, {data: dataURI, Reader: Data64URIReader, Writer: Data64URIWriter})
        ...
        createReader(reader, function (zipReader) {
            zipReader.getEntries(function (entries) {
                entries.forEach(function (entry) {
                    ...
                    }), entry.directory || addChild(parent, name, {data: entry, Reader: ZipBlobReader})
                }), onend()
            })
        }, onerror)
        ...
        for (childIndex = 0; childIndex < that.children.length; childIndex++)if (child = that.children[childIndex], child.name == name)return child
    }, FS.prototype = {
        remove: function (entry) {
            detach(entry), this.entries[entry.id] = null
            ...
            resetFS(this), this.root.importBlob(blob, onend, onerror)
        }, importText: function (text, onend, onerror) {
            resetFS(this), this.root.importText(text, onend, onerror)
        }, importData64URI: function (dataURI, onend, onerror) {
            resetFS(this), this.root.importData64URI(dataURI, onend, onerror)
    ...
    function HttpReader(url) {
        function getData(callback, onerror) {
            var request;
            that.data ? callback() : (request = new XMLHttpRequest, request.addEventListener("load", function () {
                that.size || (that.size = Number(request.getResponseHeader("Content-Length"))), that.data = new Uint8Array(request.response), callback()
            }, !1), request.addEventListener("error", onerror, !1), request.open("GET", url), request.responseType = "arraybuffer", request.send())
        }

        function init(callback, onerror) {
            var request = new XMLHttpRequest;
            request.addEventListener("load", function () {
                that.size = Number(request.getResponseHeader("Content-Length")), callback()
            }, !1), request.addEventListener("error", onerror, !1), request.open("HEAD", url), request.send()
        }
    ...
    function HttpRangeReader(url) {
        function init(callback, onerror) {
            var request = new XMLHttpRequest;
            request.addEventListener("load", function () {
                that.size = Number(request.getResponseHeader("Content-Length")), "bytes" == request.getResponseHeader("Accept-Ranges") ? callback() : onerror(ERR_HTTP_RANGE)
            }, !1), request.addEventListener("error", onerror, !1), request.open("HEAD", url), request.send()
        }

        function readArrayBuffer(index, length, callback, onerror) {
            var request = new XMLHttpRequest;
            request.open("GET", url), request.responseType = "arraybuffer", request.setRequestHeader("Range", "bytes=" + index + "-" + (index + length - 1)), request.addEventListener("load", function () {
                callback(request.response)
            }, !1), request.addEventListener("error", onerror, !1), request.send()
        }
    ...
    function ArrayBufferReader(arrayBuffer) {
        function init(callback, onerror) {
            that.size = arrayBuffer.byteLength, callback()
        }
    ...
    function ArrayBufferWriter() {
        function init(callback, onerror) {
            array = new Uint8Array, callback()
        }

        function writeUint8Array(arr, callback, onerror) {
            var tmpArray = new Uint8Array(array.length + arr.length);
            tmpArray.set(array), tmpArray.set(arr, array.length), array = tmpArray, callback()
        }
    ...
    function FileWriter(fileEntry, contentType) {
        function init(callback, onerror) {
            fileEntry.createWriter(function (fileWriter) {
                writer = fileWriter, callback()
            }, onerror)
        }

        function writeUint8Array(array, callback, onerror) {
            var blob = new Blob([appendABViewSupported ? array : array.buffer], {type: contentType});
            writer.onwrite = function () {
                writer.onwrite = null, callback()
            }, writer.onerror = onerror, writer.write(blob)
        }
    ...
    var ZipDirectoryEntry, appendABViewSupported, ERR_HTTP_RANGE = "HTTP Range not supported.", Reader = zip.Reader, Writer = zip.Writer;
    try {
        appendABViewSupported = 0 === new Blob([new DataView(new ArrayBuffer(0))]).size
    } catch (e) {
    }
    HttpReader.prototype = new Reader, HttpReader.prototype.constructor = HttpReader, HttpRangeReader.prototype = new Reader, HttpRangeReader.prototype.constructor = HttpRangeReader, ArrayBufferReader.prototype = new Reader, ArrayBufferReader.prototype.constructor = ArrayBufferReader, ArrayBufferWriter.prototype = new Writer, ArrayBufferWriter.prototype.constructor = ArrayBufferWriter, FileWriter.prototype = new Writer, FileWriter.prototype.constructor = FileWriter, zip.FileWriter = FileWriter, zip.HttpReader = HttpReader, zip.HttpRangeReader = HttpRangeReader, zip.ArrayBufferReader = ArrayBufferReader, zip.ArrayBufferWriter = ArrayBufferWriter, zip.fs && (ZipDirectoryEntry = zip.fs.ZipDirectoryEntry, ZipDirectoryEntry.prototype.addHttpContent = function (name, URL, useRangeHeader) {
        function addChild(parent, name, params, directory) {
        ...
        return addChild(this, name, {data: URL, Reader: useRangeHeader ? HttpRangeReader : HttpReader})
    }, ZipDirectoryEntry.prototype.importHttpContent = function (URL, useRangeHeader, onend, onerror) {
        this.importZip(useRangeHeader ? new HttpRangeReader(URL) : new HttpReader(URL), onend, onerror)
    }, zip.fs.FS.prototype.importHttpContent = function (URL, useRangeHeader, onend, onerror) {
        this.entries = [], this.root = new ZipDirectoryEntry(this), this.root.importHttpContent(URL, useRangeHeader, onend, onerror)
    })
}(), function () {
    "use strict";
...
EPUBJS.VERSION = "0.2.14", EPUBJS.plugins = EPUBJS.plugins || {}, EPUBJS.filePath = EPUBJS.filePath || "/epubjs/", EPUBJS.Render = {}, function (root) {
    ...
    book.contents = parse.packageContents(packageXml, book.settings.contentsPath), book.manifest = book.contents.manifest, book.spine = book.contents.spine, book.spineIndexByURL = book.contents.spineIndexByURL, book.metadata = book.contents.metadata, book.settings.bookKey || (book.settings.bookKey = book.generateBookKey(book.metadata.identifier)), book.globalLayoutProperties = book.parseLayoutProperties(book.metadata), book.contents.coverPath && (book.cover = book.contents.cover = book.settings.contentsPath + book.contents.coverPath), book.spineNodeIndex = book.contents.spineNodeIndex, book.ready.manifest.resolve(book.contents.manifest), book.ready.spine.resolve(book.contents.spine), book.ready.metadata.resolve(book.contents.metadata), book.ready.cover.resolve(book.contents.cover), book.locations = new EPUBJS.Locations(book.spine, book.store, book.settings.withCredentials), book.contents.navPath ? (book.settings.navUrl = book.settings.contentsPath + book.contents.navPath, book.loadXml(book.settings.navUrl).then(function (navHtml) {
        ...
        0 !== pageList.length && (book.pageList = book.contents.pageList = pageList, book.pageList.forEach(function (pg) {
            pg.cfi || (wait += 1, epubcfi.generateCfiFromHref(pg.href, book).then(function (cfi) {
                pg.cfi = cfi, pg.packageUrl = book.settings.packageUrl, wait -= 1, 0 === wait && (book.pagination.process(book.pageList), book.ready.pageList.resolve(book.pageList))
            }))
    ...
    return this.isRendered ? (this.renderer.removeStyle(style),
        this.renderer.reformat(), void delete this.settings.styles[style]) : this._q.enqueue("removeStyle", arguments)
...
EPUBJS.core = {}, EPUBJS.core.getEl = function (elem) {
    ...
    var rect;
    return !!(el && "function" == typeof el.getBoundingClientRect && (rect = el.getBoundingClientRect(),
    0 !== rect.width && 0 !== rect.height && rect.left >= x && x <= rect.left + rect.width))
...
EPUBJS.replace = {}, EPUBJS.replace.hrefs = function (callback, renderer) {
    ...
    function Rusha(chunkSize) {
        "use strict";
        var self$2 = {fill: 0}, padlen = function (len) {
            for (len += 9; len % 64 > 0; len += 1);
            return len
        }, padZeroes = function (bin, len) {
            for (var i = len >> 2; i < bin.length; i++)bin[i] = 0
        }, padData = function (bin, chunkLen, msgLen) {
            bin[chunkLen >> 2] |= 128 << 24 - (chunkLen % 4 << 3), bin[((chunkLen >> 2) + 2 & -16) + 15] = msgLen << 3
        }, convStr = function (str, bin, start, len) {
            var i, m = len % 4, j = len - m;
            for (i = 0; j > i; i = i + 4 | 0)bin[i >> 2] = str.charCodeAt(start + i) << 24 | str.charCodeAt(start + i + 1) << 16 | str.charCodeAt(start + i + 2) << 8 | str.charCodeAt(start + i + 3);
            switch (m) {
                case 0:
        ...
        }, convBuf = function (buf, bin, start, len) {
            var i, m = len % 4, j = len - m;
            for (i = 0; j > i; i = i + 4 | 0)bin[i >> 2] = buf[start + i] << 24 | buf[start + i + 1] << 16 | buf[start + i + 2] << 8 | buf[start + i + 3];
            switch (m) {
                case 0:
        ...
        }, conv = function (data, bin, start, len) {
            if ("string" == typeof data)return convStr(data, bin, start, len);
            if (data instanceof Array || "undefined" != typeof global && "undefined" != typeof global.Buffer && global.Buffer.isBuffer(data))return convBuf(data, bin, start, len);
            if (data instanceof ArrayBuffer)return convBuf(new Uint8Array(data), bin, start, len);
            if (data.buffer instanceof ArrayBuffer)return convBuf(new Uint8Array(data.buffer), bin, start, len);
            throw new Error("Unsupported data type.")
        }, hex = function (arrayBuffer) {
            var i, x, hex_tab = "0123456789abcdef", res = [], binarray = new Uint8Array(arrayBuffer);
            for (i = 0; i < binarray.length; i++)x = binarray[i], res[i] = hex_tab.charAt(x >> 4 & 15) + hex_tab.charAt(x >> 0 & 15);
            return res.join("")
        }, ceilHeapSize = function (v) {
            var p;
            if (65536 >= v)return 65536;
            if (16777216 > v)for (p = 1; v > p; p <<= 1); else for (p = 16777216; v > p; p += 16777216);
            return p
        }, resize = function (size) {
            if (size % 64 > 0)throw new Error("Chunk size must be a multiple of 128 bit");
            self$2.maxChunkLen = size, self$2.padMaxChunkLen = padlen(size), self$2.heap = new ArrayBuffer(ceilHeapSize(self$2.padMaxChunkLen + 320 + 20)), self$2.core = RushaCore({
                Int32Array: Int32Array,
                DataView: DataView
            }, {}, self$2.heap), self$2.buffer = null
        };
        resize(chunkSize || 4194304);
        var initState = function (heap, padMsgLen) {
            var io = new Int32Array(heap, padMsgLen + 320, 5);
            io[0] = 1732584193, io[1] = -271733879, io[2] = -1732584194, io[3] = 271733878, io[4] = -1009589776
        }, initChunk = function (chunkLen, msgLen, finalize) {
            var padChunkLen = chunkLen;
            finalize && (padChunkLen = padlen(chunkLen));
            var view = new Int32Array(self$2.heap, 0, padChunkLen >> 2);
            return finalize && padZeroes(view, chunkLen), finalize && padData(view, chunkLen, msgLen), padChunkLen
        }, coreCall = function (data, chunkStart, chunkLen, msgLen, finalize) {
            var padChunkLen = initChunk(chunkLen, msgLen, finalize), view = new Int32Array(self$2.heap, 0, padChunkLen >> 2);
            conv(data, view, chunkStart, chunkLen), self$2.core.hash(padChunkLen, self$2.padMaxChunkLen)
        }, getRawDigest = function (heap, padMaxChunkLen) {
            var io = new Int32Array(heap, padMaxChunkLen + 320, 5), out = new Int32Array(5), arr = new DataView(out.buffer);
            return arr.setInt32(0, io[0], !1), arr.setInt32(4, io[1], !1), arr.setInt32(8, io[2], !1), arr.setInt32(12, io[3], !1), arr.setInt32(16, io[4], !1), out
        }, rawDigest = this.rawDigest = function (str) {
            var msgLen = str.byteLength || str.length;
            initState(self$2.heap, self$2.padMaxChunkLen);
            var chunkOffset = 0, chunkLen = self$2.maxChunkLen;
            for (chunkOffset = 0; msgLen > chunkOffset + chunkLen; chunkOffset += chunkLen)coreCall(str, chunkOffset, chunkLen, msgLen, !1);
            return coreCall(str, chunkOffset, msgLen - chunkOffset, msgLen, !0), getRawDigest(self$2.heap, self$2.padMaxChunkLen)
        };
    ...
    if ("undefined" != typeof module && (module.exports = Rusha), "undefined" != typeof window && (window.Rusha = Rusha), "undefined" != typeof FileReaderSync) {
        var reader = new FileReaderSync, hasher = new Rusha(4194304);
        self.onmessage = function (event) {
            var hash, data = event.data.data;
            if (data instanceof Blob)try {
                data = reader.readAsBinaryString(data)
            } catch (e) {
                return void self.postMessage({id: event.data.id, error: e.name})
            }
    ...
    W.prototype.Ma = function () {
        "undefined" != typeof aa && this.a && (this.V = "Timed out after " + this.Y + "ms, aborting",
...
epubLibrary.config(function ($compileProvider) {
    $compileProvider.aHrefSanitizationWhitelist(/^\s*(https?|ftp|mailto|file|tel|filesystem|chrome-extension):/), $compileProvider.imgSrcSanitizationWhitelist(/^\s*(https?|ftp|file|filesystem|chrome-extension):|data:image\//)
}), epubLibrary.config(function ($sceDelegateProvider) {
    $sceDelegateProvider.resourceUrlWhitelist(["self", "filesystem:**", "chrome-extension:**"]), $sceDelegateProvider.resourceUrlBlacklist([])
...
epubLibrary.controller("LibraryCtrl", ["$scope", "$location", "$http", function ($scope, $location, $http) {
    angular.element(document).ready(function () {
        $scope.library = {loading: !1, progress: 0, books: []}, filer.init({
            persistent: !0,
            size: 1048576e3
        }, function (fs) {
            filer.mkdir("readiator/epub", !1, function (dirEntry) {
            }, onError), filer.mkdir("readiator/books", !1, function (dirEntry) {
            }, onError), scanBooks($scope)
        }, onError), $location.search().epub && $http({
            url: $location.search().epub,
            method: "GET",
            responseType: "blob"
        }).success(function (response) {
            var file = (document.getElementById("epub-file"), new File([response], "tmp.epub"));
        ...
        filer.cd("/", function (entries) {
            ...
            }, onError), filer.rm("readiator/epub/" + name, function () {
            }, onError)
        }, onError)
    }
    ...
    return function ($scope, element, attrs) {
        element.bind("dragover", function (evt) {
            ...
            evt.preventDefault(), $scope.$apply(function () {
